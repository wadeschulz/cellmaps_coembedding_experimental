...
This is an advanced developer section that describes creation of a new release for cellmaps_generate_ppi and deploying
...
Be sure to update the version in ``cellmaps_generate_ppi/__init__.py`` Omit the ``v`` when setting the version in the file.

Commit the above changes and push to **main**:

.. code-block::

    git commit -m 'message...' cellmaps_generate_ppi/__init__.py
...
From cellmaps_generate_ppi source tree generate the wheel and tar.gz files
...
.. code-block::
    ...
    $ tree
    dist
    |-- cellmaps_generate_ppi-0.1.0-py2.py3-none-any.whl
    `-- cellmaps_generate_ppi-0.1.0.tar.gz
...
#. From https://github.com/idekerlab/cellmaps_generate_ppi click on releases link.
...
Once the above is done verify deploy was successful by browsing to https://testpypi.python.org/pypi/cellmaps_generate_ppi and verifying new release was deployed.

Also test the package by installing it locally by running this:

.. code-block::

    pip install -i https://testpypi.python.org/pypi cellmaps_generate_ppi
...
.. code-block::
    ...
    pip install cellmaps_generate_ppi


.. _Travis: https://travis-ci.org/idekerlab/cellmaps_generate_ppi
.. _main: https://github.com/idekerlab/cellmaps_generate_ppi/tree/main
